...
Modifier cards:
  ...
  - DrawPile / IndState / CatchUp: shared; single set, expanded by print count.
"""
...
# ── Colours ──────────────────────────────────────────────────────────────────
DARK = {
    "Military": "#c53030", "Market": "#276749", "Science": "#553c9a",
    "Wonders":  "#744210", "Misc":   "#4a5568", "Action":  "#2c5282",
    "DrawPile": "#1a365d", "IndState":"#22543d","CatchUp": "#702459",
    "Player 1": "#7b241c", "Player 2": "#1a5276",
}
LIGHT = {
    "Military": "#fff5f5", "Market": "#f0fff4", "Science": "#faf5ff",
    "Wonders":  "#fffff0", "Misc":   "#f7fafc", "Action":  "#ebf8ff",
    "DrawPile": "#ebf8ff", "IndState":"#f0fff4","CatchUp": "#fff5f7",
    "Player 1": "#fdedec", "Player 2": "#eaf2ff",
}
MID = {
    "Military": "#e53e3e", "Market": "#38a169", "Science": "#805ad5",
    "Wonders":  "#d69e2e", "Misc":   "#718096", "Action":  "#3182ce",
    "DrawPile": "#2b6cb0", "IndState":"#276749","CatchUp": "#97266d",
    "Player 1": "#c0392b", "Player 2": "#2980b9",
}
...
def expand(cards):
    """Repeat each card by its 'count' field."""
    out = []
    for c in cards:
        out.extend([c] * c.get("count", 1))
    return out

def player_deck(cards, player_label):
    """Return expanded personal deck re-coloured for one player."""
    out = []
    for c in cards:
        nc = dict(c, **{"class": player_label,
                        "note": f"{player_label} · personal deck"})
        out.extend([nc] * nc.get("count", 1))
    return out
...
def svg_card(col, row, card):
    x, y   = cx(col), cy(row)
    cls    = card["class"]
    dark   = DARK[cls];  light = LIGHT[cls];  mid = MID[cls]
    era    = card.get("era", "")
    name   = card["name"]
    cost   = card.get("cost", "")
    ...
    # Note (deck label for modifier cards)
    if note:
        out.append(f'<text x="{x+22.5:.1f}" y="{y+CARD_H-3.5:.1f}" font-size="2.8" '
...
def svg_page(cards, title):
    ...
    for i, card in enumerate(cards[:PER_PAGE]):
...
def write_sheets(cards, basename, label):
    ...
    for i, page_cards in enumerate(pages):
        ...
        pg_label = label + (f" — page {i+1}/{len(pages)}" if len(pages) > 1 else "")
        write(path, svg_page(page_cards, pg_label))


# ══════════════════════════════════════════════════════════════════════════════
# EMPIRE CARD DATA  (shared market deck — 1 copy of each)
# ══════════════════════════════════════════════════════════════════════════════
...
MILITARY = [
    ...
    dict(cls="Military", name="War Camp",        era="I",
         ...
         effect="BUILDING — place: 1 mkt action + gold per track level. +1 attack action per era while on your mat."),
    ...
    dict(cls="Military", name="Castle",          era="II",
         ...
         effect="BUILDING — place: 1 mkt action + gold per track level. +2 defense to this region and all adjacent. Stacks."),
    ...
    dict(cls="Military", name="Fortress",        era="III",
         ...
         effect="BUILDING — place: 1 mkt action + gold per track level. +3 defense to this region while on your mat."),
...
MARKET = [
    ...
    dict(cls="Market", name="Mine",              era="I",
         ...
         effect="BUILDING — place: 1 mkt action + gold per track level. Produces 1 Iron per round."),
    dict(cls="Market", name="Papyrus Workshop",  era="I",
         cost="6g OR 1 food",    placement="Grassland or Swamp",
         effect="BUILDING — place: 1 mkt action + gold per track level. Produces 1 Paper per round."),
    dict(cls="Market", name="Loom",              era="II",
         cost="6g OR 1 food",    placement="Grassland",
         effect="BUILDING — place: 1 mkt action + gold per track level. Produces 1 Cloth per round."),
    dict(cls="Market", name="Iron Forge",        era="II",
         cost="10g OR 2 iron",   placement="Mountain",
         effect="BUILDING — place: 1 mkt action + gold per track level. Produces 2 Iron per round."),
    dict(cls="Market", name="Glassworks",        era="II",
         cost="8g OR 2 food",    placement="Desert or Ocean",
         effect="BUILDING — place: 1 mkt action + gold per track level. Produces 1 Glass per round."),
    dict(cls="Market", name="Scriptorium",       era="III",
         cost="10g OR 2 paper",  placement="Any region",
         effect="BUILDING — place: 1 mkt action + gold per track level. Produces 2 Paper per round."),
]
...
WONDERS = [
    ...
    dict(cls="Wonders", name="Cathedral",        era="III",
         ...
         effect="All your regions gain +1 defense while this card is on your mat.", vp="+3"),
]
...
# ══════════════════════════════════════════════════════════════════════════════
# MODIFIER CARD DATA
# count = number of physical copies per set
# ══════════════════════════════════════════════════════════════════════════════

def mod(deck_cls, name, effect, count):
    return dict(cls=deck_cls, name=name, era="", cost="", placement="—",
                effect=effect, count=count)

# Personal deck — one full set per player (24 cards/player)
PERSONAL = [
    mod("Personal", "FAIL",
        "Attack automatically fails. Reshuffle entire deck (remaining + discard + this card).", 2),
    mod("Personal", "−2",
        "Add −2 to your attack or defense strength this combat.", 2),
    mod("Personal", "−1",
        "Add −1 to your attack or defense strength this combat.", 4),
    mod("Personal", "0",
        "No modifier to strength this combat.", 8),
    mod("Personal", "+1",
        "Add +1 to your attack or defense strength this combat.", 4),
    mod("Personal", "+2",
        "Add +2 to your attack or defense strength this combat.", 2),
    mod("Personal", "SUCCESS",
        "Attack automatically succeeds. Reshuffle entire deck (remaining + discard + this card).", 2),
]
# 24 cards total per player: 2+2+4+8+4+2+2 = 24

# Modifier Draw Pile — shared (15 cards)
DRAWPILE = [
    mod("DrawPile", "FAIL",
        "Draw Pile card. If drawn during Draft or Action effect, add to player discard; reshuffle Draw Pile after.", 2),
    mod("DrawPile", "−1",
        "Modifier Draw Pile card. Added to personal modifier deck via Draft or Action cards.", 1),
    mod("DrawPile", "0",
        "Modifier Draw Pile card. Added to personal modifier deck via Draft or Action cards.", 2),
    mod("DrawPile", "+1",
        "Modifier Draw Pile card. Added to personal modifier deck via Draft or Action cards.", 3),
    mod("DrawPile", "+2",
        "Modifier Draw Pile card. Added to personal modifier deck via Draft or Action cards.", 4),
    mod("DrawPile", "SUCCESS",
        "Draw Pile card. If drawn during Draft or Action effect, add to player discard; reshuffle Draw Pile after.", 3),
]
# 15 cards total: 2+1+2+3+4+3 = 15

# Independent State Deck — shared, reshuffled fresh before every attack vs independent region (11 cards)
INDSTATE = [
    mod("IndState", "−2",
        "Independent State deck. Reshuffled before every attack against an independent region.", 1),
    mod("IndState", "−1",
        "Independent State deck. Reshuffled before every attack against an independent region.", 2),
    mod("IndState", "0",
        "Independent State deck. No Fail card in this deck.", 4),
    mod("IndState", "+1",
        "Independent State deck.", 2),
    mod("IndState", "+2",
        "Independent State deck.", 1),
    mod("IndState", "SUCCESS",
        "Attack against this independent region automatically succeeds.", 1),
]
# 11 cards: 1+2+4+2+1+1 = 11

# Catch-Up Deck — shared (12 cards, distinct colored face, remove after game)
CATCHUP = [
    mod("CatchUp", "+2",
        "Catch-Up card. Given to last-place player at end of each round. Added to modifier discard pile. Remove after game.", 6),
    mod("CatchUp", "+3",
        "Catch-Up card. Given to last-place player at end of each round. Added to modifier discard pile. Remove after game.", 5),
    mod("CatchUp", "SUCCESS",
        "Catch-Up card. Attack automatically succeeds. Remove after game.", 1),
]
# 12 cards: 6+5+1 = 12

# ── Normalise class key (svg_card uses card["class"]) ────────────────────────
for group in (MILITARY, MARKET, SCIENCE, WONDERS, MISC, ACTION,
              PERSONAL, DRAWPILE, INDSTATE, CATCHUP):
    for c in group:
        if "cls" in c:
            c["class"] = c.pop("cls")
...
# Personal modifier decks — full set (24 cards) in each player's colour
p1_cards = player_deck(PERSONAL, "Player 1")   # 24 cards, red
p2_cards = player_deck(PERSONAL, "Player 2")   # 24 cards, blue
write_sheets(p1_cards, "cards_modifiers_p1", "TOFTA — Modifier Deck: Player 1 (Red)")
write_sheets(p2_cards, "cards_modifiers_p2", "TOFTA — Modifier Deck: Player 2 (Blue)")

# Shared modifier decks — expanded by count
write_sheets(expand(DRAWPILE), "cards_modifiers_drawpile",
             "TOFTA — Modifier Draw Pile (shared, 15 cards)")
write_sheets(expand(INDSTATE), "cards_modifiers_indstate",
             "TOFTA — Independent State Deck (shared, 11 cards)")
write_sheets(expand(CATCHUP),  "cards_modifiers_catchup",
             "TOFTA — Catch-Up Deck (shared, 12 cards — remove after game)")
...
print(f"  Action:      {len(ACTION)} unique cards")
print(f"  Player 1 personal deck: {len(p1_cards)} cards")
print(f"  Player 2 personal deck: {len(p2_cards)} cards")
print(f"  Modifier Draw Pile:     {len(expand(DRAWPILE))} cards (shared)")
print(f"  Ind. State Deck:        {len(expand(INDSTATE))} cards (shared)")
print(f"  Catch-Up Deck:          {len(expand(CATCHUP))} cards (shared)")
