...
- Collects gold equal to their current gold production (this can reduce gold if gold production is negative). If collecting would take your gold below 0, you must first abandon regions until the shortfall is covered. Gold cannot go below 0.
...
|-----|-----------|--------------|
| I   | First player reaches 5 VP | Remove unpurchased Era I cards; populate market with Era II cards; recalculate gold production |
| II  | First player reaches 9 VP | Remove unpurchased Era II cards; populate market with Era III cards; recalculate gold production |
...
| Prosperous Empire | Reach a gold production of 10 |
...
The game tracks two separate quantities for each player, like Terraforming Mars:

- **Gold** — the amount of gold you currently hold. Spent on cards, tokens, and actions. Cannot go below 0.
- **Gold production** — your rate of gold generation. At the end of each round you collect gold equal to your current gold production. Can be negative (drains your gold).

Gold production is the sum of your connected regions' gold production values plus any persistent modifiers from card effects. It is tracked as a single running total on your player mat and updates immediately whenever a region is gained, lost, or abandoned — or when a card effect modifies it. Card effects that say "reduce gold production by X" or "increase gold production by X" adjust this total directly and persist until the game ends or another effect reverses them.
...
**Connectivity:** A region must have a continuous path of player-controlled regions back to the Capitol to contribute its gold production value. Disconnected regions contribute 0.

**Negative gold production:** Negative gold production drains your gold at end of round. You can sustain a deficit as long as your gold covers it. Forced abandonment only triggers when applying gold production would take your gold below 0 — at that point you must abandon regions until the shortfall is covered. Forced abandonment can cascade — dropping a region may break connectivity, reducing other regions' contributions to 0 and making recovery harder.
...
| Discipline | Round | Iron Discipline, Bulwark | Iron Discipline: this region and all adjacent controlled regions gain +1 defense. Bulwark: this region gains +2 defense. | Placing card is discarded |
...
**Canal gold production:** When a Canal is placed, count the sea tiles currently reachable through its active sides (the connected sea network at that moment, not counting the canal region itself). The canal earns that count as a permanent **+gold production bonus**, fixed at placement — it does not change if the network later grows or shrinks. A Canal Bent placed between two isolated 2-tile ocean pockets earns +4 gp; a Canal Cross placed after someone else has already expanded the same area earns more if the network is larger at that point.

**Canal connectivity is shared.** Once a Canal is built, it becomes part of the geographic sea network and any player may use that connectivity — for Naval Bastion attacks, Maritime Exchange bonuses, and sea-adjacent token bonuses. The strategic implication: building a canal into an established network rewards the latecomer (they count the full enlarged network at placement) more than the pioneer — but the pioneer controls the regions and shapes where the network can grow.
...
Any player may always convert their own stockpile via the bank at a baseline rate. This costs a market action and does not require a card:
...
**Draft** (1 market action + 3 gold): Draw 2 cards from the shared Modifier Draw Pile. Look at both privately, add one to your modifier discard pile, and return the other to the bottom of the Modifier Draw Pile. If both drawn cards are Fail, you must add one — there is no opt-out. The added card enters circulation the next time your deck reshuffles.

**Cull** (1 market action + 3 gold): Search your modifier deck and discard pile. Choose any 3 cards, inspect them, and permanently remove 1 to the Trash (out of the game). Return the other 2 to your deck and reshuffle. **Fail and Success cards cannot be culled** — they are permanent fixtures of every player's deck.
...
| Iron Discipline | II | 6 gold OR 2 stone | — | When played, choose a region you control. Place a discipline marker on it — that region and all adjacent regions you control gain +1 defense while this card is on your mat. Remove the marker if you discard this card. |
| Mercenaries | II | 6 gold OR 2 food | — | Place 2 cubes on this card when first played and at the start of each era. Spend 1 cube to gain 1 extra attack action; that attack costs 1 additional gold. Unspent cubes are removed at each era transition. |
| Vanguard's Edge | II | 6 gold OR 1 iron + 1 food | — | While on your mat: when combat results in a tie, you win as the attacker — the conquest succeeds as a clean conquest (no revolt). |
| Scouting Report | II | 5 gold OR 1 iron | — | When played, choose an enemy region adjacent to a region you control. Place an advantage marker on it — when you attack that region, draw with advantage. Remove the marker when you conquer the region or discard this card. |
...
| Border Lumber | II | 7 gold OR 1 iron + 1 wood | — | All your forest regions bordering another player produce 1 Wood per round, regardless of buildings. |
